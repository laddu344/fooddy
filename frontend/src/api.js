...
export const itemAPI = {
  ...
  updateStock: (itemId, stockStatus) =>
    api.put(`/api/item/update-stock/${itemId}`, { stockStatus }),
  searchItems: (query, city) =>
    api.get(`/api/item/search-items?query=${query}&city=${city}`),
};
...
export const orderAPI = {
  ...
  verifyPayment: (paymentData) =>
    api.post("/api/order/verify-payment", paymentData),
  ...
  updateStatus: (orderId, shopId, status) =>
    api.post(`/api/order/update-status/${orderId}/${shopId}`, { status }),
  deleteOrder: (orderId) => api.delete(`/api/order/delete-order/${orderId}`),
  cancelOrder: (orderId, reason) =>
    api.post(`/api/order/cancel-order/${orderId}`, { reason }),
};
...
export const superAdminAPI = {
  ...
  createCategory: (formData) =>
    api.post("/api/superadmin/categories", formData, {
      headers: { "Content-Type": "multipart/form-data" },
    }),
  deleteCategory: (categoryId) =>
    api.delete(`/api/superadmin/categories/${categoryId}`),
  updateCategory: (categoryId, formData) =>
    api.put(`/api/superadmin/categories/${categoryId}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    }),
  ...
  createUserType: (newUserType) =>
    api.post("/api/superadmin/user-types", newUserType),
  updateUserTypeDelivery: (userTypeId, deliveryAllowed) =>
    api.put(`/api/superadmin/user-types/${userTypeId}/delivery`, {
      deliveryAllowed,
    }),
  deleteUserType: (userTypeId) =>
    api.delete(`/api/superadmin/user-types/${userTypeId}`),
};
